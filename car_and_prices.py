...
def configure_driver():
    try:
        ...
        driver.set_page_load_timeout(10)  # Wait for 10 seconds
...
def get_car_listings(driver, url) -> list:
    try:
        logger.info(f"Accessing URL: {url}")
        driver.get(url)
        scroll_to_bottom(driver)
        soup = BeautifulSoup(driver.page_source, "html.parser")
        all_details = soup.select("div.masonry-item a.qa-advert-list-item")
        logger.info(f'Found a total of {len(all_details)} car listings')
        return all_details
    except Exception as e:
