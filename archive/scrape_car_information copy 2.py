...
def configure_driver():
    try:
        ...
        # Modify capabilities to wait for the page to load completely 
        caps = DesiredCapabilities().CHROME 
        caps["pageLoadStrategy"] = "normal" # Change to "none" if you want to skip waiting for the full page to load
...
def get_car_listings(driver, url)->list[dict]:
    try:
        ...
        for listing in soup.select("div.masonry-item a.qa-advert-list-item"):

            listing_url = 'https://jiji.ng' + listing["href"]  # Construct the full URL            
            details = get_listing_details(driver, listing_url)            
            all_details.append(details)  
            logger.info(f"Found:  { details} ")
            time.sleep(random.uniform(2, 5))
